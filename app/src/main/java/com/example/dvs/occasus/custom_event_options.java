...
class custom_event_options extends ArrayAdapter<String>{

    public custom_event_options(Context context, String options[])
    {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(position==0)
            custom_image.setImageResource(R.drawable.edit);
        else if(position==1)
            custom_image.setImageResource(R.drawable.delete);
        else if(position==2)
        custom_image.setImageResource(R.drawable.show);
        return customView;
    }
}
